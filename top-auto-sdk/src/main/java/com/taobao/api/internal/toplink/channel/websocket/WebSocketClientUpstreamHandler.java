...
public class WebSocketClientUpstreamHandler extends NettyClientUpstreamHandler {
	...
	private void handleHandshake(ChannelHandlerContext ctx, HttpResponse response) throws Exception {
		...
		boolean validUpgrade = response.headers().get(Names.UPGRADE) != null
				&& response.headers().get(Names.UPGRADE).equalsIgnoreCase(Values.WEBSOCKET);
		boolean validConnection = response.headers().get(Names.CONNECTION) != null
				&& response.headers().get(Names.CONNECTION).equalsIgnoreCase(Values.UPGRADE);

		if (!validStatus || !validUpgrade || !validConnection) {
			throw new LinkException(response.getStatus().getCode(), String.format(Text.WS_HANDSHAKE_INVALID,
					response.getContent().readable() ? response.getContent().toString(Charset.forName("UTF-8")) : ""));
		}
	...
	private void dump(HttpResponse response) {
		...
		for (Entry<String, String> h : response.headers()) {
